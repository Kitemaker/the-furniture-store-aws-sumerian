    ...
    void update() {
        ...
        for (AugmentedImage img : updatedAugmentedImages) {
            if (img.getTrackingState() == TrackingState.TRACKING) {
                try {
                    String imList[] = this.mSurfaceView.getContext().getAssets().list("awsimages");
                    for(int i = 0; i<imList.length;i++){
                        if (img.getName().equals(imList[i].split("\\.")[0])) {
                    ...
                    Log.e("AWS", "I/O exception loading augmented image bitmap.", e);
                }
            }
        }
    }
